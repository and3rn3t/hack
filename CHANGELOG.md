...
### Added - Content Expansion: Steganography, Malware Analysis, and IoT Security

- **🖼️ Steganography Challenges** (33% difficulty increase):

  - **Level 2 Steganography (5 challenges)**:
    - **LSB Steganography Basics** (`steg_lsb_basics`): Learn least significant bit manipulation in images
    - **EXIF Data Extraction** (`steg_exif_data`): Extract GPS coordinates hidden in image metadata
    - **Audio Spectrogram Analysis** (`steg_audio_spectrum`): Discover visual messages in audio waveforms
    - **Whitespace Steganography** (`steg_whitespace`): Decode binary messages hidden in spaces/tabs
    - **File Concatenation** (`steg_file_concat`): Extract archives appended to images (JPEG+ZIP polyglots)

  - **Level 3 Steganography (1 challenge)**:
    - **DNA Sequence Encoding** (`steg_dna_encoding`): Advanced biological encoding using DNA base pairs

- **🦠 Malware Analysis Challenges** (Level 3):

  - **Code Obfuscation Techniques** (`malware_obfuscation`): Deobfuscate JavaScript with hex escapes and eval chains
  - **Behavioral Analysis** (`malware_behavior`): Identify malware by analyzing system API call patterns
  - **Sandbox Evasion** (`malware_sandbox`): Recognize anti-VM techniques like timing attacks and environment detection

- **🔌 IoT Security Challenges** (Level 3):

  - **Default Credentials** (`iot_default_creds`): Exploit factory default passwords on IoT devices
  - **Firmware Extraction** (`iot_firmware`): Extract sensitive data from firmware images using binwalk patterns
  - **MQTT Protocol Analysis** (`iot_mqtt`): Intercept IoT device communications over unsecured MQTT

- **📈 Challenge Statistics**:
  - Total challenges: 36 → 48 (+33% content)
  - Level 0: 6 challenges (unchanged)
  - Level 1: 7 challenges (unchanged)
  - Level 2: 22 challenges (+7 from v1.2.0, +5 new steganography, +2 new from other categories)
  - Level 3: 12 challenges (+7 from v1.2.0: +1 stego, +3 malware, +3 iot)
  - Level 4: 1 challenge (unchanged)

- **🎓 Educational Enhancements**:
  - All new challenges include progressive hints (3-4 per challenge)
  - Comprehensive educational descriptions explaining real-world applications
  - References to industry tools (binwalk, Wireshark, stegdetect, IDA Pro)
  - Balanced difficulty progression with 10-15 sanity cost per challenge
  - XP rewards scaled appropriately (100-150 XP per challenge)

### Changed

- **Challenge Category Expansion**: Added `Malware` and `IoT` to ChallengeCategory enum
- **Challenge Distribution**: Rebalanced level progression for better educational flow
- **Version**: Bumped from 1.2.0 to 1.5.0 (skipped 1.3.0, 1.4.0 for major content release)

### Testing

- ✅ All 81 tests passing
- ✅ Challenge count validation tests updated
- ✅ Level distribution tests updated for new challenge structure
- ✅ All new challenges compile and validate correctly
- ✅ Educational content reviewed for accuracy and accessibility
